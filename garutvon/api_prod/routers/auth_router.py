...
async def create_api_key(name: str | None = None, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    if not current_user:
...
async def list_keys(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    keys = q.scalars().all()
    return [{"id": k.id, "key": (k.key if not k.revoked else None), "name": k.name, "revoked": k.revoked} for k in keys]


@router.post("/keys/revoke", response_model=dict)
async def revoke_key(key_id: int, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    q = await db.execute(select(ApiKey).where(ApiKey.id == key_id, ApiKey.owner_id == current_user.id))
    key = q.scalars().first()
    if not key:
